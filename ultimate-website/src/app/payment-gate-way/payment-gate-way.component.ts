...
export class PaymentGateWayComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscriptions.push(this.paymentGateWayService
  ...
  public proceed(paymentDetails : any) {
    ...
    if(account.title){
      this.RAZORPAY_OPTIONS.name = account.name;
    } else this.RAZORPAY_OPTIONS.name = "Company";
    if(account.image){
      this.RAZORPAY_OPTIONS.image = account.image;
  ...
  public razorPaySuccessHandler(response) {
    ...
    this.subscriptions.push(this.paymentGateWayService.placePurchaseorder(this.paymentInfo).subscribe((res: any) => {
  ...
  setType(paymentDetails :any){
    ...
    this.paymentData = '';
    this.type = '';
    this.user = this.storage.get('studentData');
  ...
  setPayemtDetailsToLocalStorage(){
    var ciphertext = CryptoJS.AES.encrypt(JSON.stringify(this.paymentInfo), environment.en_key).toString();
    localStorage.setItem('paymentDetails',ciphertext);
    // localStorage.setItem('paymentDetails', JSON.stringify(this.paymentInfo));
  }
