...
export class ContentDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(public redirectionService: RedirectionService, public httpURL: HttpClient, public router: Router, private sanitizer: DomSanitizer, private route: ActivatedRoute, protected serviceContentDetails: ContentDetailsApiHelper, private toastr: ToastrService, public paymentGateWayService: PaymentGateWayService, private location: Location, public progressLogService: ProgressLogService) {
    // this.contentId = this.route.snapshot.paramMap.get('id');
    this.subscriptions.push(this.redirectionService.getContentDetailsUrl().subscribe(data => {
      this.contentId = data.id;
      this.fromType = data.type;
    }));
    if (!this.contentId) {
      const ids = JSON.parse(localStorage.getItem('id'));
      if (ids.contentDetailsId) {
        this.contentId = ids.contentDetailsId;
      }
    }
  }
    ...
    this.subscriptions.push(this.serviceContentDetails.getContentDetails(contentData).subscribe((res: ServerResponse) => {
      ...
      if (res.success && res.data != null) {
        ...
        this.contentDetailsList.audio.forEach((audio: any) => {
          ...
          // this.getAudioLength(audio.url).then(function (length: any) {
          //   var time = length;
          //   audio.length = length;
          //   audio.hr = Math.floor(time / 3600).toString();
          //   if (audio.hr.length == 1) {
          //     audio.hr = "0" + audio.hr;
          //   }
          //   audio.min = Math.floor(time % 3600 / 60).toString();
          //   if (audio.min.length == 1) {
          //     audio.min = "0" + audio.min;
          //   }
          //   audio.sec = Math.floor(time % 3600 % 60).toString();
        ...
        this.contentDetailsList.video.forEach((video: any) => {
          ...
          if (video.type == 0) {
            ...
            this.serviceContentDetails.getVideoData(id).subscribe((res: any) => {
  ...
  goToContent() {
    this.router.navigate(['./student/content/']).then(() => {
      ...
      if (this.fromType == "topic") {
        this.redirectionService.sendContentUrl({ 'id': this.redirectId.contentId, 'type': this.fromType });
      }
  ...
  VideoSelect(VideoIndex) {
    ...
    if (this.videoSelected.videoType == 0) {
      if (this.contentDetailsList.video[VideoIndex].hlsURL) {
        ...
        setTimeout(() => {
          ...
          // videotime.setAttribute('src', 'https://bjn-videoshare-prod.s3.us-west-2.amazonaws.com/3848780/trans/0-709323491Media11d1d358-6eae-4906-932c-11f2571439ea_360.mp4?response-content-disposition=attachment%3Bfilename%3Drecording.mp4&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20210513T060350Z&X-Amz-SignedHeaders=host&X-Amz-Expires=259200&X-Amz-Credential=AKIAXXXXXXXXEXAMPLE3%2F20210513%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Signature=64af8a176292840dd1e165f7e61a879c1469f1ffec79c5dcd81559049c9e9982');
    ...
    } else if (this.videoSelected.videoType == 3) {
      this.isMp4 = false;
      setTimeout(() => {
        BufferM3U8Video(this.contentDetailsList.video[VideoIndex].url);
        let videotime: any = document.getElementById('html-player');
        videotime.setAttribute('src', this.contentDetailsList.video[VideoIndex].url);
        console.log(videotime);
        if (this.contentDetailsList.video[VideoIndex].videoProgress) {
          videotime.currentTime = this.contentDetailsList.video[VideoIndex].videoProgress.secounds;
        }
  ...
  testNavigate(testDetails: any) {
    ...
    if (testDetails.testResult != null) {
      ...
      // this.router.navigate(['/student/taketest/' + testDetails.id]);
    } else {
  ...
  practiceTestNavigation(practiceTest) {
    this.router.navigate(['./student/practice']).then(() => {
      this.redirectId = JSON.parse(localStorage.getItem('id'));
      this.redirectId.practiceTestId = practiceTest.id;
