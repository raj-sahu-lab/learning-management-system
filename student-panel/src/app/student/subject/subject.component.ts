...
export class SubjectComponent implements OnInit, OnDestroy {
  ...
  constructor( public redirectionService: RedirectionService, public router: Router, private route: ActivatedRoute, protected serviceSubject: SubjectApiHelper, private toastr: ToastrService, public paymentGateWayService : PaymentGateWayService, private location: Location, public helperService: HelperService) {
  ...
  navigateUrl(topic : any){
    if(!topic.isPaid){
      this.router.navigate(['./student/content/']).then(()=>{
        ...
        this.redirectId.contentId = topic.id;
    ...
    this.subscriptions.push(this.paymentGateWayService.checkCoupon(data).subscribe((res: ServerResponse) => {
      if (res && res.success) {
        if(res.data.currency.code == this.subjectList.currency.code){
          ...
          this.toastr.error('Coupon currency not match with subject currency');
        }
      }
    },
      (err) => {
