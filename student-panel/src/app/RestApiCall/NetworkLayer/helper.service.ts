...
export class HelperService {
  ...
  //to change the dashboard feed when institute is selected
  private _invokeInstituteEvent = new Subject<any>();

  listenInstitute(): Observable<any> {
    return this._invokeInstituteEvent.asObservable();
  }

  userInstitute(institute: string) {
    this._invokeInstituteEvent.next(institute);
  }

  //to change the profile pic in header when changed in setting page
  private _invokeProfileChangeEvent = new Subject<any>();

  listenImage(): Observable<any> {
    return this._invokeProfileChangeEvent.asObservable();
  }

  userImage() {
    this._invokeProfileChangeEvent.next();
  }

  //to change the profile pic in header when changed in setting page
  private _invokeToken = new Subject<any>();

  listenToken(): Observable<any> {
    return this._invokeToken.asObservable();
  }

  sendToken() {
    this._invokeToken.next();
  }
