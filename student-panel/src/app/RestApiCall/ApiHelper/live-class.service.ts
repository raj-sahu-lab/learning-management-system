...
export class LiveClassService {
  ...
  getSignature(meetingDetails: any): Observable<any> {
    ...
    this.branchId = this.user.branch.id;
    meetingDetails.branchId = this.branchId;
    ...
    return this.http.post(appApiResources.liveClassSignature, meetingDetails, headers).pipe(
      map((res: Response) => res),
      catchError((error: HttpErrorResponse) => throwError(error.error))
    );
