...
export class CurrencyApiHelper {
  ...
  getCurrency(): Observable<any> {
    ...
    return this.baseService.get(appApiResources.currency, headers).pipe(
      map((res: Response) => res),
      catchError((error: HttpErrorResponse) => throwError(error.error) )
    );
  }

  primaryCurrency(currencyData: any): Observable<any> {
    
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + localStorage.getItem('logintoken'));

    return this.baseService.put(appApiResources.defaultCurrency, currencyData, headers).pipe(
        map((res: Response) => res),
        catchError((error: HttpErrorResponse) => throwError(error.error))
      );
  }
