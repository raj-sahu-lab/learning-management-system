...
export class forgetPasswordApiHelper {
  ...
  constructor(@Inject(SESSION_STORAGE) private storage: WebStorageService, public http: BaseService, public helperService: HelperService, public errorHandler: CustomErrorHandlerService) {}
   ...
   return this.http.post(appApiResources.forGotPassword, phoneNumber, headers).pipe(
  ...
  changePassword(passwordData: any): Observable<any> {
    ...
    return this.http.post(appApiResources.changePassword, passwordData, headers).pipe(
         map((res: Response) => res),
         catchError((error: HttpErrorResponse) => throwError(error.error))
       );
   }
