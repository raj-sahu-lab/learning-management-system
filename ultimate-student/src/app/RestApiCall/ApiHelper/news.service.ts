...
export class NewsService {
  ...
  getNewsCategory(): Observable<any> {
    ...
    return this.http.get(appApiResources.newsCategory, headers).pipe(
      map((res: Response) => res),
      catchError((error: HttpErrorResponse) => throwError(error.error) )
    );
  }

  getNews( newsData : any) : Observable<any> {
    ...
    return this.http.post(appApiResources.news, newsData, headers).pipe(
      map((res: Response) => res),
      catchError((error: HttpErrorResponse) => throwError(error.error))
    );
