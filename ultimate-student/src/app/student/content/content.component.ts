...
export class ContentComponent implements OnInit, OnDestroy {
  ...
  constructor(@Inject(SESSION_STORAGE) private storage: WebStorageService, public helperService : HelperService, public redirectionService: RedirectionService, public router: Router, private route: ActivatedRoute, protected serviceContent: ContentApiHelper, public paymentGateWayService : PaymentGateWayService, private location: Location, private toastr: ToastrService) {
    ...
    if(!this.topicId){
      if(this.storage.get('contentId')){
      this.topicId = this.storage.get('contentId');
      }
    }
  }   
  ...
  navigateUrl(content : any){
    ...
    if(!content.isPaid){
      this.router.navigate(['./student/content-details/']).then(()=>{
        this.storage.set('contentDetailsId',content.id);
