...
export class PollsComponent implements OnInit, OnDestroy {
  ...
  constructor(@Inject(SESSION_STORAGE) private storage: WebStorageService, public redirectionService: RedirectionService, private sanitizer: DomSanitizer,private route: ActivatedRoute, private toastr: ToastrService, public pollsService: PollsService, private location: Location) {
    this.subscriptions.push(this.redirectionService.getPollUrl().subscribe(id =>{
      this.pollId = id;
    }));
    if(!this.pollId){
      if(this.storage.get('pollId')){
      this.pollId = this.storage.get('pollId');
      }
    }
   }
