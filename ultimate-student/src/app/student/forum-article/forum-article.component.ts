...
export class ForumArticleComponent implements OnInit, OnDestroy {
  ...
  constructor(@Inject(SESSION_STORAGE) private storage: WebStorageService, public toastr : ToastrService, public discussionForumService: DiscussionForumService, public redirectionService: RedirectionService, public router : Router, public route: ActivatedRoute,protected serviceForum: ForumService, private location: Location) { 
    this.subscriptions.push(this.redirectionService.getForumArtUrl().subscribe(id =>{
      this.articalId = id;
    }));
    if(!this.articalId){
      if(this.storage.get('forumArtId')){
      this.articalId = this.storage.get('forumArtId');
      }
    }
  }
