...
module.exports.authUser = async function (userInfo) {
    ...
    if (!user) TE('Invalid Credentials. Please try again. If account created by institute check your mail and verify the mail');
                    ...
                    include:[
                        {
                            ...
                            as: 'account',
                            where: { status: 0, delete: 0 },
                            attributes: [['account_id', 'id'], ['account_title', 'title'] , ['account_image', 'image'] , ['is_marketplace_enable' , 'is_MarketplaceEnable'],['account_phone', 'phone'] ,['account_email', 'email'], 'accessLevel'],
                            include:[
                                {
                                    model: TOA_country,
                                    as: 'country',
                ...
                include:[
                    {
                        model: TOA_account,
                        as: 'account',
                        attributes: [['account_id', 'id'], ['account_title', 'title'] , ['account_phone', 'phone'] ,['account_email', 'email'], ['account_image', 'image'], ['is_marketplace_enable' , 'is_MarketplaceEnable'], 'accessLevel' ],
                        include:[
                            {
                                model: TOA_country,
                                as: 'country',
