const purchaseService = require('../../../services/V1/Student/Purchase.service');
...
module.exports.verifyCouponcode = async function (req, res) {
    ...
    if (!body.accountId) {
       ...
       [err, couponObj] = await to(purchaseService.getCoupon(body.accountId, body.type, body.typeId, body.couponCode));
...
module.exports.addStripeAndroidPurchase = async function (req, res) {
    ...
    if (!body.accountId) {
        ...
        const paymentIntent = await stripeInit.paymentIntents.create({
            amount: body.amount * 100,
            currency: body.currency,
            description: "Buy through android"
        });
        let data = {
            clientSecret : paymentIntent.client_secret
        }
        ReS(res, 'client secret sent successfully', data);
